...
class ElectrumClient extends Client{
    ...
    blockchainTransaction_verbose(tx_hash, vin_page = 0, vin_offset = 20, enable_vin_details = 1){
        return this.request('blockchain.transaction.verbose', [tx_hash, vin_page, vin_offset, enable_vin_details])
    }
    blockchainTransaction_fullVerbose(tx_hash){
        return this.request('blockchain.transaction.vin', [tx_hash])
    }
}
